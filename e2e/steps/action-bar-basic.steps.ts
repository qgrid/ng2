...
When('I scroll page right [{int}]', async (num: number) => scrollElementHorizontalOnValue(await element.all(by.tagName('tbody')).get(0).getWebElement(), num));
When('I scroll page left [{int}]', async (num: number) => scrollElementHorizontalOnValue(await element.all(by.tagName('tbody')).get(0).getWebElement(), -num));
When('I sort column {string} downwards', async (col: string) => sortColumnDownwards(col));
