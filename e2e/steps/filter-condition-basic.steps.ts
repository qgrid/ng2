...
async function setField(field) {
	if (field !== `Number`) {
		const defaultFields = element.all(by.xpath(`//span[contains(@class,'mat-select-value-text') and contains(.,'Number')]`));
		await defaultFields
...
async function setOperator(operator) {
	if (operator !== `EQUALS`) {
		const defaultOperators = element.all(by.xpath(`//span[contains(@class,'mat-select-value-text') and contains(.,'EQUALS')]`));
		await defaultOperators
			.get(await defaultOperators.count() - 1)
			.click();
		const opt = element.all(by.xpath(`//*[contains(@class,'mat-option-text') and contains(.,' ${operator} ')]`));
	...
	if (value !== ``) {
		if (operator === `BETWEEN`) {
			...
			values.forEach(async elem => {
				await valueInput.get(am - 1).sendKeys(elem.trim(), protractor.Key.ENTER);
			});
		} else {
			const valueInput = element(by.className(`q-grid-query-builder-expression`))
				.all(by.css(`input[placeholder='Select value']`));
...
async function setOperation(andOR) {
	const defaultOperator = element.all(by.xpath(`//span[contains(@class,'mat-select-value-text') and contains(.,'AND')]`));
